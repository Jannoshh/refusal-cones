...
def run_pipeline(model_path):
    ...
    best_config = direction_evaluations[0]
    ...
    import wandb
    api = wandb.Api()
    group = f"sb_data_with_retain_{model_alias}"
    run_name = "run_5"
    project = "refusal-representations/robust_refusal_vector"
    ...
    download_dir = os.path.join("wandb_downloads", group, run_name)
    os.makedirs(download_dir, exist_ok=True)
    latest_run.file(file_path).download(root=download_dir, exist_ok=True)
    ...
    # generate_and_save_completions_for_dataset(cfg, model_base, baseline_fwd_pre_hooks, baseline_fwd_hooks, 'baseline', "xstest")
    generate_and_save_completions_for_dataset(cfg, model_base, ablation_fwd_pre_hooks, ablation_fwd_hooks, 'ablation', "xstest")
    # TODO: save location is wrong when not usng DIM

    # 3b. Evaluate completions and save results on harmful evaluation datasets
    # evaluate_completions_and_save_results_for_dataset(cfg, 'baseline', "xstest", eval_methodologies=["xstest"])
    evaluate_completions_and_save_results_for_dataset(cfg, 'ablation', "xstest", eval_methodologies=["xstest"])
