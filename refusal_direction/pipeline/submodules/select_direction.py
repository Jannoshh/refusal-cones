...
def select_direction_directopt(
    model_base: ModelBase,
    harmful_instructions,
    harmless_instructions,
...
def select_subspace_direction_directopt2(
    model_base: ModelBase,
    harmful_instructions,
    harmless_instructions,
    ...
    for candidate_idx in range(n_candidates):
        ...
        if not all_basis_discarded:
            ...
            discard_basis = False # fuck the basis if nothing works
            print("="*100)
        ...
        filtered_scores.append((sorting_score, candidate_idx)) # TODO need to change sorting score
...
def select_subspace_direction_directopt(
    ...
    for candidate_idx in range(n_candidates):
        ...
        json_output_all_scores.append({
            'candidate_idx': candidate_idx,
            'layer': add_layer,
            'refusal_score': ablation_refusal_scores[candidate_idx, :].tolist(),
            'steering_score': steering_refusal_scores[candidate_idx, :].tolist(),
            'kl_div_score': ablation_kl_div_scores[candidate_idx, :].tolist()
        })

        refusal_score = ablation_refusal_scores[candidate_idx, :].max()
        steering_score = steering_refusal_scores[candidate_idx, :].min()
        kl_div_score = ablation_kl_div_scores[candidate_idx, :].max()
        ...
        json_output_filtered_scores.append({
            'candidate_idx': candidate_idx,
            'layer': add_layer,
            'max_refusal_score': ablation_refusal_scores[candidate_idx, :].max().item(),
            'mean_refusal_score': ablation_refusal_scores[candidate_idx, :].mean().item(),
            'min_steering_score': steering_refusal_scores[candidate_idx, :].min().item(),
            'mean_steering_score': steering_refusal_scores[candidate_idx, :].mean().item(),
            'max_steering_score': steering_refusal_scores[candidate_idx, :].max().item(),
            'max_kl_div_score': ablation_kl_div_scores[candidate_idx, :].max().item(),
            'mean_kl_div_score': ablation_kl_div_scores[candidate_idx, :].mean().item(),
            'min_kl_div_score': ablation_kl_div_scores[candidate_idx, :].min().item()
        })   
    ...
    best_direction = candidate_directions[candidate_idx, :]
    norm_best_direction = best_direction / best_direction.norm(dim=-1, keepdim=True)
    n_samples = 128
    ...
    sample_steering_refusal_scores = torch.zeros((n_samples), device=model_base.model.device, dtype=torch.float64)

    for sample_idx in tqdm(range(n_samples), desc=f"Computing KL divergence for sample directions"):
        sample_vector = transformed_samples[sample_idx]
        fwd_pre_hooks = [(model_base.model_block_modules[layer], get_direction_ablation_input_pre_hook(direction=sample_vector)) for layer in range(model_base.model.config.num_hidden_layers)]
        fwd_hooks = [(model_base.model_attn_modules[layer], get_direction_ablation_output_hook(direction=sample_vector)) for layer in range(model_base.model.config.num_hidden_layers)]
        fwd_hooks += [(model_base.model_mlp_modules[layer], get_direction_ablation_output_hook(direction=sample_vector)) for layer in range(model_base.model.config.num_hidden_layers)]
        ...
        sample_ablation_kl_div_scores[sample_idx] = kl_div_fn(baseline_harmless_logits, intervention_logits, mask=None).mean(dim=0).item()

    for sample_idx in tqdm(range(n_samples), desc=f"Computing refusal ablation for sample directions"):
        sample_vector = transformed_samples[sample_idx]
        fwd_pre_hooks = [(model_base.model_block_modules[layer], get_direction_ablation_input_pre_hook(direction=sample_vector)) for layer in range(model_base.model.config.num_hidden_layers)]
        fwd_hooks = [(model_base.model_attn_modules[layer], get_direction_ablation_output_hook(direction=sample_vector)) for layer in range(model_base.model.config.num_hidden_layers)]
        fwd_hooks += [(model_base.model_mlp_modules[layer], get_direction_ablation_output_hook(direction=sample_vector)) for layer in range(model_base.model.config.num_hidden_layers)]
        ...
        refusal_scores = get_refusal_scores(model_base.model, harmful_instructions, model_base.tokenize_instructions_fn, model_base.refusal_toks, fwd_pre_hooks=fwd_pre_hooks, fwd_hooks=fwd_hooks, batch_size=batch_size)
        sample_ablation_refusal_scores[sample_idx] = refusal_scores.mean().item()
    
    coeff = torch.tensor(1.0)
    for sample_idx in tqdm(range(n_samples), desc=f"Computing steering refusal for sample directions"):
        sample_vector = transformed_samples[sample_idx]
        fwd_pre_hooks = [(model_base.model_block_modules[add_layer], get_activation_addition_input_pre_hook(vector=sample_vector, coeff=coeff))]
        fwd_hooks = []

        refusal_scores = get_refusal_scores(model_base.model, harmless_instructions, model_base.tokenize_instructions_fn, model_base.refusal_toks, fwd_pre_hooks=fwd_pre_hooks, fwd_hooks=fwd_hooks, batch_size=batch_size)
        sample_steering_refusal_scores[sample_idx] = refusal_scores.mean().item()
    ...
    for sample_idx in range(n_samples):
        ...
        json_output_all_scores.append({
            ...
            'layer': add_layer,
            'refusal_score': sample_ablation_refusal_scores[sample_idx].item(),
            'steering_score': sample_steering_refusal_scores[sample_idx].item(),
            'kl_div_score': sample_ablation_kl_div_scores[sample_idx].item()
        })
        ...
        sample_filtered_scores.append((sorting_score, sample_idx))

        json_output_filtered_scores.append({
            'sample_idx': sample_idx,
            'layer': add_layer,
            'max_refusal_score': sample_ablation_refusal_scores[sample_idx].item(),
            'mean_refusal_score': sample_ablation_refusal_scores[sample_idx].item(),
            'min_steering_score': sample_steering_refusal_scores[sample_idx].min().item(),
            'mean_steering_score': sample_steering_refusal_scores[sample_idx].mean().item(),
            'max_steering_score': sample_steering_refusal_scores[sample_idx].max().item(),
            'max_kl_div_score': sample_ablation_kl_div_scores[sample_idx].max().item(),
            'mean_kl_div_score': sample_ablation_kl_div_scores[sample_idx].mean().item(),
            'min_kl_div_score': sample_ablation_kl_div_scores[sample_idx].min().item()
        })   
    ...
    # sorted in descending order
    sample_filtered_scores = sorted(sample_filtered_scores, key=lambda x: x[0], reverse=True)

    # now return the best position, layer, and direction
    score, sample_idx = sample_filtered_scores[0]

    print(f"Selected direction: candidate_idx={candidate_idx}")
    print(f"Refusal scores: {ablation_refusal_scores[candidate_idx, :].tolist()}")
    print(f"Mean Refusal score: {ablation_refusal_scores[candidate_idx, :].mean().item():.4f} (baseline: {baseline_refusal_scores_harmful.mean().item():.4f})")
    print(f"Max Refusal score: {ablation_refusal_scores[candidate_idx, :].max().item():.4f}")
    print(f"Steering scores: {steering_refusal_scores[candidate_idx, :].tolist()}")
    print(f"Mean Steering score: {steering_refusal_scores[candidate_idx, :].mean().item():.4f} (baseline: {baseline_refusal_scores_harmless.mean().item():.4f})")
    print(f"Min Steering score: {steering_refusal_scores[candidate_idx, :].min().item():.4f}")
    print(f"KL Divergence scores: {ablation_kl_div_scores[candidate_idx, :].tolist()}")
    print(f"Mean KL Divergence: {ablation_kl_div_scores[candidate_idx, :].mean().item():.4f}")
    print(f"Max KL Divergence: {ablation_kl_div_scores[candidate_idx, :].max().item():.4f}")
    print(f"Selected sample direction: sample_idx={sample_idx}")
    print(f"Refusal score: {sample_ablation_refusal_scores[sample_idx].item()}")
    print(f"Steering score: {sample_steering_refusal_scores[sample_idx].item()}")
    print(f"KL Divergence score: {sample_ablation_kl_div_scores[sample_idx].item()}")
    print("Sample Refusal scores:")
    print(f"Mean sample Refusal score: {sample_ablation_refusal_scores.mean().item():.4f}")
    print(f"Std sample Refusal score: {sample_ablation_refusal_scores.std().item():.4f}")
    print(f"Min sample Refusal score: {sample_ablation_refusal_scores.min().item():.4f}")
    print(f"Max sample Refusal score: {sample_ablation_refusal_scores.max().item():.4f}")
    print("Sample Steering scores:")
    print(f"Mean sample Steering score: {sample_steering_refusal_scores.mean().item():.4f}")
    print(f"Std sample Steering score: {sample_steering_refusal_scores.std().item():.4f}")
    print(f"Min sample Steering score: {sample_steering_refusal_scores.min().item():.4f}")
    print(f"Max sample Steering score: {sample_steering_refusal_scores.max().item():.4f}")
    print("Sample KL Divergence scores:")
    print(f"Mean sample KL Divergence score: {sample_ablation_kl_div_scores.mean().item():.4f}")
    print(f"Std sample KL Divergence score: {sample_ablation_kl_div_scores.std().item():.4f}")
