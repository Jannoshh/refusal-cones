...
def run_pipeline(model_path):
    ...
    token = best_config["position"]
    direction = candidate_directions[token, layer]

    alpha = direction.norm().item()
    import numpy as np
    alphas = np.linspace(0.0, alpha, 16)
    direction = direction / direction.norm()
    # direction = torch.load("../vectors/rdo_qwen3b.pt")[-20 + 19]
    
    # 3a. Generate and save completions on harmful evaluation datasets
    for dataset_name in cfg.evaluation_datasets:
        for alpha in alphas:
            alpha = int(alpha)
            actadd_fwd_pre_hooks, actadd_fwd_hooks = [(model_base.model_block_modules[layer], get_activation_addition_input_pre_hook(vector=direction, coeff=alpha))], []
            generate_and_save_completions_for_dataset(cfg, model_base, actadd_fwd_pre_hooks, actadd_fwd_hooks, f'actadd_{alpha}', dataset_name)

    # 3b. Evaluate completions and save results on harmful evaluation datasets
    for dataset_name in cfg.evaluation_datasets:
        for alpha in alphas:
            alpha = int(alpha)
            evaluate_completions_and_save_results_for_dataset(cfg, f'actadd_{alpha}', dataset_name, eval_methodologies=cfg.refusal_eval_methodologies)
