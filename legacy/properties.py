...
# harmless_train = harmless_train[:len(harmful_val)]
# print(len(harmful_val), len(harmless_train))
...
# Plot each individual sample's score as a separate line
sample_scores_matrix = np.array(sample_scores_matrix)  # shape: (len(alphas), n_samples)
sample_scores_matrix = sample_scores_matrix.T  # shape: (n_samples, len(alphas)); each row corresponds to one sample's scores across alphas
plt.figure(figsize=(10, 6))
for i in range(n_samples):
    plt.plot(alphas, sample_scores_matrix[i], alpha=0.7)
plt.xlabel('Alpha')
plt.ylabel('Refusal Score')
plt.title('Refusal Score per Sample vs Alpha')
plt.grid(True)
